...
class JitsiIntegration:
    ...
    def __init__(self, config: dict):
        ...
        # Feature flags
        self.auto_create_rooms = config.get(
            'integrations.jitsi.auto_create_rooms', True
        )
        self.enable_recording = config.get(
            'integrations.jitsi.enable_recording', False
        )
        self.enable_lobby = config.get(
            'integrations.jitsi.enable_lobby', False
        )
        ...
        if self.enabled:
            ...
            self.logger.info(
                f"Jitsi Meet integration enabled (Server: {self.server_url})"
            )
    ...
    def create_meeting(
        ...
        if not self.enabled:
            return {
                'success': False,
                'error': 'Jitsi integration is disabled'
            }
        ...
        try:
            ...
            self.logger.info(
                f"Jitsi meeting created: {room_name} ({meeting_url})"
            )
    ...
    def get_active_participants(self, room_name: str) -> List[str]:
        ...
        self.logger.debug(
            f"Participant list for {room_name} requires "
            "self-hosted Jitsi with API access"
        )
        return []

    def create_conference_bridge(
        self,
        conference_id: str,
