...
class MatrixIntegration:
    ...
    def __init__(self, config: dict):
        ...
        self.homeserver_url = config.get(
            'integrations.matrix.homeserver_url',
            'https://matrix.org'
        )
        ...
        self.notification_room = config.get(
            'integrations.matrix.notification_room'
        )
        self.voicemail_room = config.get(
            'integrations.matrix.voicemail_room'
        )
        ...
        if self.enabled:
            if not REQUESTS_AVAILABLE:
                ...
                self.logger.error(
                    "Matrix integration requires homeserver_url, "
                    "bot_username, and bot_password"
                )
                self.enabled = False
            else:
                self.logger.info(
                    f"Matrix integration enabled (Server: {self.homeserver_url})"
                )
                # Authenticate bot
                self._authenticate()
    ...
    def _authenticate(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.logger.error(
                    f"Matrix authentication failed: {response.status_code}"
                )
                return False
