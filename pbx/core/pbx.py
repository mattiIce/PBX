...
class PBXCore:
    ...
    def __init__(self, config_file="config.yml"):
        ...
        if self.database.connect():
            ...
            self.logger.warning(
                "Database backend not available - running without database")
            self.logger.warning(
                "Extensions will be loaded from config.yml only")
            self.logger.warning(
                "Voicemails will be stored ONLY as files (no database metadata)")
        ...
        self.voicemail_system = VoicemailSystem(
            ...
            config=self.config,
            database=self.database if hasattr(
                self,
                'database') and self.database.enabled else None)
            ...
            if self.ad_integration.enabled:
                ...
                if self.ad_integration.auto_provision:
                    ...
                    try:
                        ...
                        synced_count = sync_result if isinstance(
                            sync_result, int) else sync_result.get(
                            'synced_count', 0)
        ...
        if self.config.get('security.threat_detection.enabled', True):
            ...
            self.threat_detector = get_threat_detector(database=self.database if hasattr(
                self, 'database') and self.database.enabled else None, config=self.config)
        ...
        if self.config.get('features.dnd_scheduling.enabled', False):
            ...
            if hasattr(
                    self,
                    'integrations') and 'outlook' in self.integrations:
                outlook = self.integrations['outlook']

            self.dnd_scheduler = get_dnd_scheduler(
                presence_system=self.presence_system if hasattr(
                    self,
        ...
        if self.config.get('features.skills_routing.enabled', False):
            ...
            self.skills_router = get_skills_router(database=self.database if hasattr(
                self, 'database') and self.database.enabled else None, config=self.config)
    ...
    def _auto_seed_critical_extensions(self):
        ...
        for ext_config in critical_extensions:
            ...
            try:
                ...
                self.logger.error(
                    f"Failed to auto-seed extension {number}: {e}")

        if seeded_count > 0:
            self.logger.info(
                f"Auto-seeded {seeded_count} critical extension(s) at startup")
    ...
    def _load_provisioning_devices(self):
        ...
        for device_config in devices_config:
            ...
            if all([mac, extension, vendor, model]):
                try:
                    self.phone_provisioning.register_device(
                        mac, extension, vendor, model)
                    self.logger.info(
                        f"Loaded provisioning device {mac} for extension {extension}")
                except Exception as e:
                    self.logger.error(
                        f"Failed to load provisioning device {mac}: {e}")
        ...
        if match:
            ...
            if self.extension_db:
                try:
                    ...
                    if db_extension:
                        ...
                        self.logger.debug(
                            f"Extension {extension_number} found in database")
                        ...
                        if not self.extension_registry.get(extension_number):
                            ...
                            # helper method
                            extension_obj = ExtensionRegistry.create_extension_from_db(
                                db_extension)
                    ...
                    self.logger.debug(
                        f"Error checking extension in database: {e}")
            ...
            if not extension_exists:
                ...
                if extension:
                    extension_exists = True
                    self.logger.debug(
                        f"Extension {extension_number} found in config")

            if extension_exists:
                self.extension_registry.register(extension_number, addr)
                self.logger.info(
                    f"Extension {extension_number} registered from {addr}")
                ...
                if self.registered_phones_db:
                    ...
                    mac_address = self._extract_mac_address(
                        contact, user_agent)
                    ...
                    try:
                        ...
                        self.logger.error(
                            f"Failed to store phone registration in database: {e}")
                ...
                self.logger.warning(
                    f"Unknown extension {extension_number} attempted registration")
                return False
    ...
    def _extract_mac_address(self, contact, user_agent):
        ...
        if contact:
            ...
            mac_match = re.search(
                r'mac=([0-9a-fA-F:]{17}|[0-9a-fA-F-]{17})', contact)
            if mac_match:
            ...
            instance_match = re.search(
                r'sip\.instance="<urn:uuid:([0-9a-f-]+)>"',
                contact,
                re.IGNORECASE)
        ...
        if not mac_address and user_agent:
            ...
            # 00:15:65:12:34:56"
            mac_match = re.search(
                r'([0-9a-fA-F]{2}[:-]){5}[0-9a-fA-F]{2}', user_agent)
            if mac_match:
    ...
    def route_call(self, from_header, to_header, call_id, message, from_addr):
        ...
        if self.auto_attendant and to_ext == self.auto_attendant.get_extension():
            return self._handle_auto_attendant(
                from_ext, to_ext, call_id, message, from_addr)
        ...
        if to_ext.startswith(
                '*') and len(to_ext) >= 4 and len(to_ext) <= 5 and to_ext[1:].isdigit():
            return self._handle_voicemail_access(
                from_ext, to_ext, call_id, message, from_addr)

        # Check if this is a paging call (7xx pattern or all-call)
        if self.paging_system and self.paging_system.is_paging_extension(
                to_ext):
            return self._handle_paging(
                from_ext, to_ext, call_id, message, from_addr)
        ...
        if is_webrtc_destination:
            ...
            self.logger.info(
                f"Routing call to WebRTC extension {to_ext} (session: {session_id})")
            ...
            if self.webrtc_gateway:
                ...
                success = self.webrtc_gateway.receive_call(
                    ...
                    webrtc_signaling=self.webrtc_signaling if hasattr(
                        self,
                        'webrtc_signaling') else None)

                if success:
                    self.logger.info(
                        f"Call {call_id} routed to WebRTC session {session_id}")
                    ...
                    self.logger.error(
                        f"Failed to route call to WebRTC session {session_id}")
                    return False
            else:
                self.logger.error(
                    "WebRTC gateway not available for routing call")
                return False
        ...
        if rtp_ports:
            ...
            invite_to_callee.set_header(
                'Contact', f"<sip:{from_ext}@{server_ip}:{self.config.get('server.sip_port', 5060)}>")
            invite_to_callee.set_header('Content-Type', 'application/sdp')

            # Send to destination
            self.sip_server._send_message(
                invite_to_callee.build(), dest_ext_obj.address)
            ...
            no_answer_timeout = self.config.get(
            ...
            self.logger.info(
                f"Started no-answer timer ({no_answer_timeout}s) for call {call_id}")
    ...
    def handle_callee_answer(self, call_id, response_message, callee_addr):
        ...
        if call.rtp_ports and call.caller_addr:
            ...
            if call.original_invite:
                ...
                # Send to caller
                self.sip_server._send_message(
                    ok_response.build(), call.caller_addr)
    ...
    def end_call(self, call_id):
        ...
        if call:
            ...
            if call.routed_to_voicemail and hasattr(
                    call, 'voicemail_recorder'):
                ...
                if recorder and recorder.running:
                    ...
                    if audio_data and len(audio_data) > 0:
                        ...
                        self.logger.warning(
                            f"No audio recorded for voicemail on call {call_id}")
    ...
    def handle_dtmf_info(self, call_id, dtmf_digit):
        ...
        if not call:
            ...
            self.logger.debug(
                f"Received DTMF INFO for ended/unknown call {call_id} - ignoring")
            return
        ...
        if hasattr(call, 'voicemail_ivr') and call.voicemail_ivr:
            ...
            self.logger.debug(
                f"Queued DTMF '{dtmf_digit}' from SIP INFO for call {call_id}")
    ...
    def transfer_call(self, call_id, new_destination):
        ...
        if not self.extension_registry.is_registered(new_destination):
            self.logger.error(
                f"Transfer destination {new_destination} not registered")
            return False
        ...
        refer_msg.set_header('Refer-To',
                             f"<sip:{new_destination}@{server_ip}>")

        # Add Referred-By header
        refer_msg.set_header('Referred-By',
                             f"<sip:{call.to_extension}@{server_ip}>")
    ...
    def _handle_no_answer(self, call_id):
        ...
        if not call:
            self.logger.warning(
                f"No-answer timeout for non-existent call {call_id}")
            return
        ...
        if call.state == CallState.CONNECTED:
            self.logger.debug(
                f"Call {call_id} already answered, ignoring no-answer timeout")
            return
        ...
        if hasattr(
                call,
                'callee_addr') and call.callee_addr and hasattr(
                call,
                'callee_invite') and call.callee_invite:
            ...
            cancel_request.set_header(
                'Via', call.callee_invite.get_header('Via'))

            self.sip_server._send_message(
                cancel_request.build(), call.callee_addr)
            self.logger.info(
        ...
        if call.original_invite and call.caller_addr and call.caller_rtp and call.rtp_ports:
            ...
            self.sip_server._send_message(
                ok_response.build(), call.caller_addr)
            self.logger.info(
                f"Answered call {call_id} for voicemail recording")
            ...
            if call.caller_rtp:
                try:
                    ...
                    if player.start():
                        ...
                        mailbox = self.voicemail_system.get_mailbox(
                            call.to_extension)
                        ...
                        try:
                            ...
                            if temp_file_created:
                                try:
                                    ...
                                    self.logger.debug(
                                        f"Could not delete temp greeting file: {e}")

                        # Play beep tone (1000 Hz, 500ms)
                        player.play_beep(frequency=1000, duration_ms=500)
                        player.stop()
                        self.logger.info(
                            f"Played voicemail greeting and beep for call {call_id}")
                    else:
                        self.logger.warning(
            ...
            if recorder.start():
                ...
                self.logger.error(
                    f"Failed to start voicemail recorder for call {call_id}")
    ...
    def _monitor_voicemail_dtmf(self, call_id, call, recorder):
        ...
        try:
            ...
            self.logger.info(
                f"Started DTMF monitoring for voicemail recording on call {call_id}")
            ...
            while recorder.running and call.state.value != 'ended':
                ...
                if hasattr(
                        recorder,
                        'recorded_data') and recorder.recorded_data:
                    ...
                    if len(recorder.recorded_data) > 0:
                        ...
                        if len(recent_audio) > MIN_AUDIO_BYTES_FOR_DTMF:
                            ...
                            chunk_size = min(
                                len(recent_audio), 8192)  # Process up to 8KB at once
            ...
            self.logger.debug(
                f"DTMF monitoring ended for voicemail recording on call {call_id}")
    ...
    def _complete_voicemail_recording(self, call_id):
        ...
        if not call:
            self.logger.warning(
                f"Cannot complete voicemail for non-existent call {call_id}")
            return
    ...
    def _handle_auto_attendant(
        ...
        if message.body:
            ...
            if caller_sdp:
                ...
                if caller_codecs:
                    self.logger.info(
                        f"Auto attendant: Caller codecs: {caller_codecs}")
        ...
        try:
            ...
            self.logger.error(
                f"No available RTP ports for auto attendant {call_id}")
            return False
    ...
    def _auto_attendant_session(self, call_id, call, session):
        ...
        try:
            ...
            if not call.caller_rtp:
                self.logger.warning(
                    f"No caller RTP info for auto attendant {call_id}")
                return
            ...
            if not player.start():
                self.logger.error(
                    f"Failed to start RTP player for auto attendant {call_id}")
                return
            ...
            if not dtmf_listener.start():
                self.logger.error(
                    f"Failed to start DTMF listener for auto attendant {call_id}")
                player.stop()
                return
            self.logger.info(
            ...
            self.logger.info(
                f"[Auto Attendant] Starting audio playback for call {call_id}")
            audio_played = False

            if audio_file and os.path.exists(audio_file):
                self.logger.info(
                    f"[Auto Attendant] Playing welcome file: {audio_file}")
                audio_played = player.play_file(audio_file)
                if audio_played:
                    self.logger.info(
                        f"[Auto Attendant] ✓ Welcome audio played successfully")
                else:
                    self.logger.error(
                        f"[Auto Attendant] ✗ Failed to play welcome audio")
            else:
                # Try to load from auto_attendant/welcome.wav, fallback to tone
                # generation
                self.logger.info(
                    f"[Auto Attendant] Generating welcome prompt audio")
                prompt_data = get_prompt_audio(
                    'welcome', prompt_dir='auto_attendant')
                with tempfile.NamedTemporaryFile(suffix='.wav', delete=False) as temp_file:
            ...
            # Play main menu
            self.logger.info(
                f"[Auto Attendant] Playing main menu for call {call_id}")
            menu_audio = self.auto_attendant._get_audio_file('main_menu')
            if menu_audio and os.path.exists(menu_audio):
                self.logger.info(
                    f"[Auto Attendant] Playing menu file: {menu_audio}")
                audio_played = player.play_file(menu_audio)
                if audio_played:
                    self.logger.info(
                        f"[Auto Attendant] ✓ Menu audio played successfully")
                else:
                    self.logger.error(
                        f"[Auto Attendant] ✗ Failed to play menu audio")
            else:
                # Try to load from auto_attendant/main_menu.wav, fallback to
                # tone generation
                self.logger.info(
                    f"[Auto Attendant] Generating menu prompt audio")
                prompt_data = get_prompt_audio(
                    'main_menu', prompt_dir='auto_attendant')
                with tempfile.NamedTemporaryFile(suffix='.wav', delete=False) as temp_file:
                    temp_file.write(prompt_data)
                    temp_file_path = temp_file.name
                try:
                    ...
                    if audio_played:
                        ...
                        self.logger.error(
                            f"[Auto Attendant] ✗ Failed to play generated menu audio")
                finally:
                    try:
                        os.unlink(temp_file_path)
                    except BaseException:
            ...
            while session_active and (time.time() - start_time) < timeout:
                ...
                if hasattr(call, 'dtmf_info_queue') and call.dtmf_info_queue:
                    ...
                    self.logger.info(
                        f"Auto attendant received DTMF from SIP INFO: {digit}")
                else:
                ...
                if digit:
                    ...
                    # Handle the input
                    result = self.auto_attendant.handle_dtmf(
                    ...
                    if action == 'transfer':
                        ...
                        self.logger.info(
                            f"Auto attendant transferring to {destination}")

                        # Play transfer message
                        transfer_audio = self.auto_attendant._get_audio_file(
                            'transferring')
                        ...
                        if call_id:
                            ...
                            self.logger.warning(
                                "Cannot transfer call: no call_id available")
            ...
            if time.time() - start_time >= timeout:
                ...
                if action == 'transfer':
                    destination = result.get('destination')
                    self.logger.info(
                        f"Auto attendant timeout, transferring to {destination}")
                    if call_id:
                        success = self.transfer_call(call_id, destination)
                        if not success:
                            self.logger.warning(
    ...
    def _handle_voicemail_access(
        ...
        self.logger.info(
            f"[VM Access] Step 1: Verifying target extension {target_ext} exists")
        extension_exists = False
        ...
        if self.extension_registry.get(target_ext):
            extension_exists = True
            self.logger.info(
                f"[VM Access] ✓ Extension {target_ext} found in registry")
        # Fallback to config check for backwards compatibility
        elif self.config.get_extension(target_ext):
            extension_exists = True
            self.logger.info(
                f"[VM Access] ✓ Extension {target_ext} found in config file")
        ...
        self.logger.info(
            f"[VM Access] Step 2: Loading voicemail box for extension {target_ext}")
        ...
        if message.body:
            ...
            if caller_sdp:
                ...
                self.logger.warning(
                    f"[VM Access] ⚠ No audio info found in caller SDP")
        else:
            self.logger.warning(f"[VM Access] ⚠ No SDP body in INVITE message")

        # Create call for voicemail access
        self.logger.info(
            f"[VM Access] Step 4: Creating call object in call manager")
        call = self.call_manager.create_call(
            call_id, from_ext, f"*{target_ext}")
        call.start()
        call.original_invite = message
        call.caller_addr = from_addr
        call.caller_rtp = caller_sdp
        ...
        # Start CDR record for analytics
        self.logger.info(
            f"[VM Access] Step 5: Starting CDR record for analytics")
        ...
        if rtp_ports:
            call.rtp_ports = rtp_ports
            self.logger.info(
                f"[VM Access] ✓ RTP ports allocated: {rtp_ports[0]}/{rtp_ports[1]}")
        else:
            self.logger.error(
        ...
        # Send 200 OK to answer the call
        self.logger.info(
            f"[VM Access] Step 8: Building and sending 200 OK response")
        ok_response = SIPMessageBuilder.build_response(
            200,
            "OK",
            call.original_invite,
        ...
        self.logger.info(
            f"[VM Access] ✓ Voicemail IVR created for extension {target_ext}")

        # Get message count
        self.logger.info(
            f"[VM Access] Step 10: Checking voicemail message count")
    ...
    def _handle_paging(self, from_ext, to_ext, call_id, message, from_addr):
        ...
        if not page_id:
            self.logger.error(
                f"Failed to initiate page from {from_ext} to {to_ext}")
            return False
        ...
        if rtp_ports:
            ...
            self.logger.error(
                f"Failed to allocate RTP ports for paging {call_id}")
        ...
        if not zones:
            self.logger.error(
                f"No zones configured for paging extension {to_ext}")
            self.paging_system.end_page(page_id)
            return False
        ...
        self.sip_server._send_message(ok_response.build(), call.caller_addr)
        self.logger.info(
            f"Answered paging call {call_id} - Paging {page_info.get('zone_names')}")
        ...
        if dac_device:
            ...
            self.logger.warning(
                f"Paging call {call_id} connected but no DAC device available")
            self.logger.info(
            ...
            if call.caller_rtp:
                self.logger.info(
                    f"Caller RTP: {
                        ...
                        call.caller_rtp['port']}")
                self.logger.info(
                    f"Audio relay: Caller -> PBX:{call.rtp_ports[0]} -> DAC:{dac_ip}")
    ...
    def _playback_voicemails(self, call_id, call, mailbox, messages):
        ...
        try:
            ...
            if not call.caller_rtp:
                self.logger.warning(
                    f"No caller RTP info for voicemail playback {call_id}")
            ...
            if not player.start():
                self.logger.error(
                    f"Failed to start RTP player for voicemail playback {call_id}")
                time.sleep(2)
                self.end_call(call_id)
                return
                ...
                if not messages:
                    ...
                    for idx, message in enumerate(messages):
                        ...
                        if player.play_file(file_path):
                            ...
                            self.logger.warning(
                                f"Failed to play voicemail: {file_path}")
    ...
    def _voicemail_ivr_session(self, call_id, call, mailbox, voicemail_ivr):
        ...
        try:
            ...
            if not call.caller_rtp:
                self.logger.warning(
                    f"[VM IVR] ✗ No caller RTP info available - cannot proceed")
                time.sleep(2)
                self.end_call(call_id)
                return
            self.logger.info(
            ...
            self.logger.info(
                f"[VM IVR] Creating RTP player for audio prompts...")
            ...
            # PIN, etc.)
            self.logger.info(
                f"[VM IVR] Creating DTMF detector (sample_rate=8000Hz)...")
            dtmf_detector = DTMFDetector(sample_rate=8000)
            ...
            try:
                ...
                # tone generation
                self.logger.info(
                    f"[VM IVR] Loading audio prompt: {prompt_type}")
                ...
                time.sleep(0.5)
                self.logger.info(
                    f"[VM IVR] Post-prompt pause complete, checking call state...")
                ...
                self.logger.info(f"")
                self.logger.info(
                    f"[VM IVR] ✓ IVR fully started and ready for user input")
                self.logger.info(
                    f"[VM IVR] Extension: {
                        call.voicemail_extension}")
                self.logger.info(f"[VM IVR] State: {call.state}")
                ...
                while ivr_active:
                    ...
                    if call.state == CallState.ENDED:
                        self.logger.info(
                            f"[VM IVR] Call {call_id} ended - exiting IVR loop")
                        break
                    ...
                    # reliable)
                    if hasattr(
                            call, 'dtmf_info_queue') and call.dtmf_info_queue:
                        digit = call.dtmf_info_queue.pop(0)
                        self.logger.info(
                            f"[VM IVR] >>> DTMF RECEIVED (SIP INFO): '{digit}' <<<")
                    else:
                        ...
                        if hasattr(recorder,
                            ...
                            if len(recorder.recorded_data) > 0:
                                ...
                                if len(
                                        recent_audio) > MIN_AUDIO_BYTES_FOR_DTMF:  # Need sufficient audio for DTMF
                                    try:
                                        # Detect DTMF in audio with error
                                        # handling
                                        digit = dtmf_detector.detect(
                                            recent_audio)
                                    except Exception as e:
                                        self.logger.error(
                                            f"Error detecting DTMF: {e}")
                                        digit = None
                                    ...
                                    if digit:
                                        ...
                                        if digit == last_detected_digit and (
                                                current_time - last_detection_time) < DTMF_DEBOUNCE_SECONDS:
                    ...
                    if digit:
                        ...
                        if action['action'] == 'play_message':
                            ...
                            self.logger.info(
                                f"[VM IVR] Playing prompt: {prompt_type}")
                            ...
                            try:
                                player.play_file(prompt_file)
                                self.logger.info(
                                    f"[VM IVR] ✓ Prompt '{prompt_type}' played")
                            finally:
                                try:
                                    os.unlink(prompt_file)
                                except OSError:
                            ...
                            while recording and ivr_active:
                                ...
                                # Priority 2: Check in-band audio
                                    ...
                                    recent_audio = b''.join(
                                        recorder.recorded_data[-DTMF_DETECTION_PACKETS:])
                                    if len(
                                            recent_audio) > MIN_AUDIO_BYTES_FOR_DTMF:
                                        try:
                                            stop_digit = dtmf_detector.detect(
                                                recent_audio)
                                            if stop_digit:
                                                self.logger.info(
                                ...
                                if stop_digit == '#':
                                    ...
                                    if action.get('action') == 'play_prompt':
                                        ...
                                        prompt_type = action.get(
                                            'prompt', 'greeting_review_menu')
                                        prompt_audio = get_prompt_audio(
                                            prompt_type)
                                        with tempfile.NamedTemporaryFile(suffix='.wav', delete=False) as temp_file:
                                            temp_file.write(prompt_audio)
                                            prompt_file = temp_file.name
                            ...
                            if greeting_data:
                                ...
                                review_prompt = get_prompt_audio(
                ...
                self.logger.info(
                    f"[VM IVR] Cleaning up RTP player and recorder...")
                player.stop()
                recorder.stop()
            ...
            try:
                ...
                self.logger.error(
                    f"[VM IVR] Error ending call during cleanup: {e}")
    ...
    def get_ad_integration_status(self):
        ...
        if self.extension_db:
            try:
                ...
                self.logger.error(
                    f"Error getting AD-synced extension count: {e}")
    ...
    def sync_ad_users(self):
        ...
        try:
            ...
            sync_result = self.ad_integration.sync_users(
                extension_registry=self.extension_registry,
                extension_db=self.extension_db,
                phone_provisioning=self.phone_provisioning if hasattr(
                    self,
                    'phone_provisioning') else None)
            ...
            if isinstance(sync_result, int):
                ...
                extensions_to_reboot = sync_result.get(
                    'extensions_to_reboot', [])
            ...
            if hasattr(
                    self,
                    'phone_book') and self.phone_book and self.phone_book.enabled:
            ...
            rebooted_count = 0
            if extensions_to_reboot and hasattr(
                    self, 'phone_provisioning') and self.phone_provisioning:
                self.logger.info(
                ...
                for extension_number in extensions_to_reboot:
                    try:
                        if self.phone_provisioning.reboot_phone(
                                extension_number, self.sip_server):
                            rebooted_count += 1
