...
class StatisticsEngine:
    ...
    def _get_overview_stats(self, days):
        ...
        for i in range(days):
            ...
            answered_calls += sum(1 for r in records if r.get('disposition')
                                  == 'answered')
            missed_calls += sum(1 for r in records if r.get('disposition')
                                in ['no_answer', 'busy'])
            total_duration += sum(r.get('duration', 0) for r in records)

        avg_call_duration = (total_duration /
                             answered_calls) if answered_calls > 0 else 0
        answer_rate = (
            answered_calls /
            total_calls *
    ...
    def _get_daily_trends(self, days):
        ...
        for i in range(days - 1, -1, -1):  # Reverse order for chronological
            ...
            total = len(records)
            answered = sum(1 for r in records if r.get(
                'disposition') == 'answered')
            missed = sum(1 for r in records if r.get(
                'disposition') in ['no_answer', 'busy'])
            failed = sum(1 for r in records if r.get(
                'disposition') == 'failed')
        ...
        top_callers = [
            {
                ...
                'avg_duration': round(
                    stats['duration'] /
                    stats['calls'],
                    2) if stats['calls'] > 0 else 0} for ext,
            stats in sorted(
                caller_stats.items(),
    ...
    def _get_call_disposition(self, days):
        ...
        return [{'disposition': disp,
                 ...
                 'percentage': round((count / total * 100),
                                     2) if total > 0 else 0} for disp,
                count in dispositions.items()]
    ...
    def _get_average_metrics(self, days):
        ...
        for i in range(days):
            ...
            total_calls += len(records)
            total_answered += sum(1 for r in records if r.get('disposition')
                                  == 'answered')
            total_duration += sum(r.get('duration', 0) for r in records)
    ...
    def get_call_quality_metrics(self, pbx_core=None):
        ...
        if pbx_core and hasattr(pbx_core, 'qos_monitor'):
            ...
            if historical:
                ...
                avg_mos = sum(m['mos_score']
                              for m in historical) / len(historical)
                avg_jitter = sum(m['jitter_avg_ms']
                                 for m in historical) / len(historical)
                avg_packet_loss = sum(m['packet_loss_percentage']
                                      for m in historical) / len(historical)
                avg_latency = sum(m['latency_avg_ms']
                                  for m in historical) / len(historical)

                # Calculate quality distribution
                excellent = sum(1 for m in historical if m['mos_score'] >= 4.3)
                good = sum(
                    1 for m in historical if 4.0 <= m['mos_score'] < 4.3)
                fair = sum(
                    1 for m in historical if 3.6 <= m['mos_score'] < 4.0)
                poor = sum(
                    1 for m in historical if 3.1 <= m['mos_score'] < 3.6)
    ...
    def get_real_time_metrics(self, pbx_core):
        ...
        registered_extensions = len([ext for ext in pbx_core.extensions.values(
        ) if ext.get('registered', False)]) if hasattr(pbx_core, 'extensions') else 0
    ...
    def get_advanced_analytics(self, start_date, end_date, filters=None):
        ...
        answered_calls = sum(
            1 for r in all_records if r.get('disposition') == 'answered')
        missed_calls = sum(
            1 for r in all_records if r.get('disposition') in [
                'no_answer', 'busy'])
        failed_calls = sum(
            1 for r in all_records if r.get('disposition') == 'failed')
    ...
    def get_call_center_metrics(self, days=7, queue_name=None):
        ...
        if queue_name:
            all_records = [
                r for r in all_records if r.get('queue') == queue_name]

        answered = [r for r in all_records if r.get(
            'disposition') == 'answered']
        abandoned = [r for r in all_records if r.get(
            'disposition') in ['no_answer', 'busy']]
        ...
        return {
            ...
            'average_handle_time': round(
                aht,
                2),
            'average_speed_of_answer': round(
                asa,
                2),
            'service_level_20s': round(
                service_level,
                2),
            'abandonment_rate': round(
                ...
                2),
            'answer_rate': round(
                (answered_count / total_calls * 100) if total_calls > 0 else 0,
                2)}
