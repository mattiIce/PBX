...
class StatisticsEngine:
    ...
    def get_call_quality_metrics(self, pbx_core=None):
        ...
        if pbx_core and hasattr(pbx_core, 'qos_monitor'):
            ...
            if historical:
                ...
                avg_packet_loss = sum(m['packet_loss_percentage'] for m in historical) / len(historical)
    ...
    def get_advanced_analytics(self, start_date, end_date, filters=None):
        ...
        if filters:
            ...
            if 'disposition' in filters and filters['disposition']:
                filtered_records = [r for r in filtered_records 
                                   if r.get('disposition') == filters['disposition']]
        ...
        return {
            ...
            'summary': {
                ...
                'total_duration_hours': round(total_duration / 3600, 2),
                'avg_call_duration': round((total_duration / answered_calls) if answered_calls > 0 else 0, 2)
            },
            'records': all_records,
    ...
    def get_call_center_metrics(self, days=7, queue_name=None):
        ...
        aht = sum(r.get('duration', 0) for r in answered) / answered_count if answered_count > 0 else 0
        
        # Average speed of answer (ASA) - wait time before answer
        # Note: This requires wait_time field in CDR which may not be present
        asa = sum(r.get('wait_time', 0) for r in answered) / answered_count if answered_count > 0 else 0
        
        # Service level - % answered within threshold (typically 20 seconds)
        service_threshold = 20  # seconds
        answered_within_threshold = sum(1 for r in answered if r.get('wait_time', 0) <= service_threshold)
        service_level = (answered_within_threshold / answered_count * 100) if answered_count > 0 else 0
    ...
    def export_to_csv(self, records, filename):
        ...
        try:
            ...
            # Define CSV headers
            headers = ['timestamp', 'from_ext', 'to_ext', 'caller_id', 'called_number',
                      'disposition', 'duration', 'wait_time', 'queue']
