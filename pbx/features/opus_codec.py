...
class OpusCodec:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        if self.sample_rate not in self.SAMPLE_RATES:
            self.logger.warning(f"Invalid sample rate {self.sample_rate}, using {self.DEFAULT_SAMPLE_RATE}")
        ...
        if self.complexity < 0 or self.complexity > 10:
            self.logger.warning(f"Invalid complexity {self.complexity}, using {self.DEFAULT_COMPLEXITY}")
        ...
        self.logger.info(f"Opus codec initialized: {self.sample_rate}Hz, {self.bitrate}bps, {self.frame_size}ms frames")
    ...
    def encode(self, pcm_data: bytes) -> Optional[bytes]:
        ...
        try:
            ...
            if len(pcm_data) != expected_bytes:
                self.logger.warning(f"PCM data size mismatch: got {len(pcm_data)}, expected {expected_bytes}")
                return None
