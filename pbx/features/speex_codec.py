...
class SpeexCodec:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        if self.mode not in self.SAMPLE_RATES:
            self.logger.warning(
                f"Invalid Speex mode {self.mode}, using {self.DEFAULT_MODE}")
        ...
        # Payload type
        self.payload_type = self.config.get(
            'payload_type', self.PAYLOAD_TYPES[self.mode])
    ...
    def create_encoder(self):
        ...
        try:
            ...
            self.logger.debug(
                f"Speex encoder created (mode: {self.mode}, quality: {self.quality})")
    ...
    def encode(self, pcm_data: bytes) -> Optional[bytes]:
        ...
        if len(pcm_data) != expected_bytes:
            self.logger.error(
                f"Invalid PCM data size: got {len(pcm_data)}, "
...
class SpeexCodecManager:
    ...
    def remove_codec(self, call_id: str):
        ...
        """
        if not self.enabled:
            self.logger.error(f"Cannot remove codec: Speex codec feature is not enabled")
            return False
