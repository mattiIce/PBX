...
class HubSpotIntegration:
    ...
    def sync_contact(self, contact_data: Dict) -> bool:
        ...
        try:
            ...
            if response.status_code in [200, 201]:
                self._log_activity('hubspot', 'sync_contact', 'success', f"Contact synced: {contact_data.get('email')}")
                return True
            else:
                self._log_activity('hubspot', 'sync_contact', 'error', f"API error: {response.status_code}")
    ...
    def create_deal(self, deal_data: Dict) -> bool:
        ...
        try:
            ...
            if response.status_code in [200, 201]:
                self._log_activity('hubspot', 'create_deal', 'success', f"Deal created: {deal_data.get('dealname')}")
                return True
            else:
                self._log_activity('hubspot', 'create_deal', 'error', f"API error: {response.status_code}")
                self.logger.error(f"HubSpot API error: {response.status_code} - {response.text}")
                return False
    ...
    def create_ticket(self, ticket_data: Dict) -> Optional[str]:
        ...
        try:
            ...
            if response.status_code in [200, 201]:
                ...
                self._log_activity('zendesk', 'create_ticket', 'success', f"Ticket created: {ticket_id}")
                return ticket_id
            else:
                self._log_activity('zendesk', 'create_ticket', 'error', f"API error: {response.status_code}")
    ...
    def update_ticket(self, ticket_id: str, update_data: Dict) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                self._log_activity('zendesk', 'update_ticket', 'success', f"Ticket updated: {ticket_id}")
                return True
            else:
                self._log_activity('zendesk', 'update_ticket', 'error', f"API error: {response.status_code}")
                self.logger.error(f"Zendesk API error: {response.status_code} - {response.text}")
                return False
