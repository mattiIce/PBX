...
class WebRTCSession:
    ...
    def __init__(self, session_id: str, extension: str,
...
class WebRTCSignalingServer:
    ...
    def __init__(self, config=None, pbx_core=None):
        ...
        self.rtp_port_min = self._get_config(
            'features.webrtc.rtp.port_min', 10000)
        self.rtp_port_max = self._get_config(
            'features.webrtc.rtp.port_max', 20000)
        self.rtp_packet_time = self._get_config(
            'features.webrtc.rtp.packet_time', 20)

        # NAT configuration (matches Zultys ZIP33G)
        self.nat_udp_update_time = self._get_config(
        ...
        self.audio_noise_reduction = self._get_config(
        ...
        self.audio_vad = self._get_config(
            'features.webrtc.audio.voice_activity_detection', True)
        self.audio_comfort_noise = self._get_config(
        ...
        if self.enabled:
            ...
            if self.verbose_logging:
                ...
                self.logger.info(
                    f"  TURN servers: {len(self.turn_servers)} configured")
    ...
    def create_session(self, extension: str) -> WebRTCSession:
        ...
        self.logger.info(
            f"Created WebRTC session: {session_id} (extension: {extension})")
        ...
        if self.verbose_logging:
            ...
            self.logger.info(
                f"  Peer Connection ID: {
                    session.peer_connection_id}")
            self.logger.info(f"  Total active sessions: {len(self.sessions)}")
            self.logger.info(
                f"  Sessions for extension {extension}: {
                    len(
                        self.extension_sessions.get(
                            extension,
                            set()))}")

        # Register the extension in the extension registry if PBX core is
        # available
        if self.pbx_core:
            try:
                # Check if extension exists in registry
                ext_obj = self.pbx_core.extension_registry.get_extension(
                    extension)

                if not ext_obj and extension.startswith('webrtc-'):
                    # Auto-create virtual WebRTC extension if it doesn't exist
                    # This allows WebRTC clients to connect without
                    # pre-configuring the extension
                    self.logger.info(
                        f"Auto-creating virtual WebRTC extension: {extension}")
                    ...
                    ext_obj = Extension(
                        extension,
                        virtual_ext_config['name'],
                        virtual_ext_config)
                    self.pbx_core.extension_registry.extensions[extension] = ext_obj
                    self.logger.info(
                        f"Created virtual extension {extension} in registry")

                    if self.verbose_logging:
                        self.logger.info(
                            f"[VERBOSE] Virtual extension created:")
                        self.logger.info(f"  Extension: {extension}")
                        self.logger.info(f"  Type: Virtual WebRTC")

                if ext_obj:
                    # Register as active with a virtual WebRTC address
                    # Use session_id as the unique identifier for the WebRTC
                    # connection
                    webrtc_addr = ('webrtc', session_id)
                    self.pbx_core.extension_registry.register(
                        extension, webrtc_addr)
                    self.logger.info(
                        f"Registered WebRTC extension {extension} in extension registry")
                    ...
                    if hasattr(
                        ...
                        try:
                            ...
                            # Use hash of session_id for MAC to avoid
                            # collisions while keeping reasonable length
                            import hashlib
                            mac_suffix = hashlib.sha256(
                                session_id.encode()).hexdigest()[:12]
                            ...
                            if success:
                                ...
                                if self.verbose_logging:
                                    self.logger.info(
                                        f"[VERBOSE] Phone DB registration successful")
                            else:
                                self.logger.warning(
                                    f"Failed to register WebRTC session in phones database")
                        except Exception as e:
                            self.logger.error(
                                f"Error registering WebRTC session in phones database: {e}")
                else:
                    self.logger.warning(
                        f"Extension {extension} not found in registry for WebRTC session")
                    if self.verbose_logging:
                        self.logger.warning(
                            f"[VERBOSE] Available extensions: {
                                list(
                                    self.pbx_core.extension_registry.extensions.keys())[
                                    :10]}")
            except Exception as e:
    ...
    def get_extension_sessions(self, extension: str) -> list:
        ...
        with self.lock:
            ...
            return [self.sessions[sid]
                    for sid in session_ids if sid in self.sessions]
            ...
            if last_session:
                ...
                if self.pbx_core:
                    try:
                        ...
                        if self.verbose_logging:
                            self.logger.info(
                                f"[VERBOSE] Extension unregistered:")
                            self.logger.info(f"  Extension: {extension}")
                            self.logger.info(
                                f"  Reason: Last WebRTC session closed")
                    except Exception as e:
                        self.logger.error(
                            f"Error unregistering WebRTC extension: {e}")

        self.logger.info(
            f"Closed WebRTC session: {session_id} (extension: {extension})")
    ...
    def handle_offer(self, session_id: str, sdp: str) -> bool:
        ...
        if not session:
            self.logger.warning(
                f"Received offer for unknown session: {session_id}")
            if self.verbose_logging:
    ...
    def handle_answer(self, session_id: str, sdp: str) -> bool:
        ...
        if not session:
            self.logger.warning(
                f"Received answer for unknown session: {session_id}")
            return False
    ...
    def add_ice_candidate(self, session_id: str, candidate: Dict) -> bool:
        ...
        if not session:
            self.logger.warning(
                f"Received ICE candidate for unknown session: {session_id}")
            return False
        ...
        if self.verbose_logging:
            ...
            self.logger.info(
                f"  SDP M-Line Index: {candidate.get('sdpMLineIndex', 'N/A')}")
            self.logger.info(
                f"  Total candidates for session: {len(session.ice_candidates)}")
    ...
    def set_session_call_id(self, session_id: str, call_id: str) -> bool:
        ...
        session.update_activity()
        self.logger.info(
            f"Associated call {call_id} with WebRTC session {session_id}")
        return True
...
class WebRTCGateway:
        ...
        try:
            ...
            for media in sdp.media:
                ...
                if 'DTLS' in original_protocol:
                    ...
                    self.logger.debug(
                        f"Converted protocol from {original_protocol} to RTP/AVP")
                ...
                for attr in original_attrs:
                    ...
                    if attr_name not in webrtc_attrs:
                        ...
                        self.logger.debug(
                            f"Filtered WebRTC attribute: {attr_name}")
        ...
        try:
            ...
            for media_idx, media in enumerate(sdp.media):
                ...
                if media.get('protocol') == 'RTP/AVP':
                    ...
                    self.logger.debug(
                        f"Converted protocol to RTP/SAVPF for WebRTC")
                ...
                self.logger.debug(
                    f"Added WebRTC attributes to media {media_idx}")
    ...
    def initiate_call(
            ...
            webrtc_signaling=None) -> Optional[str]:
        """
        ...
        self.logger.info(
            f"Initiating call from WebRTC session {session_id} to {target_extension}")
        ...
        try:
            ...
            target_ext_obj = self.pbx_core.extension_registry.get_extension(
                target_extension)
            ...
            if not target_ext_obj:
                is_valid_dialplan = self.pbx_core._check_dialplan(
                    target_extension)
            ...
            if not target_ext_obj and not is_valid_dialplan:
                ...
                if self.verbose_logging:
                    self.logger.error(
                        f"[VERBOSE] Extension validation failed:")
                    all_exts = list(
                        self.pbx_core.extension_registry.extensions.keys()) if hasattr(
                        self.pbx_core.extension_registry, 'extensions') else []
                    self.logger.error(
                        f"  Available extensions: {all_exts[:10]}{'...' if len(all_exts) > 10 else ''}")
                    self.logger.error(
                        f"  Dialplan check result: {is_valid_dialplan}")
                return None
            ...
            if self.verbose_logging:
                self.logger.info(
                    f"[VERBOSE] Creating call through CallManager:")
            ...
            if session.local_sdp:
                if self.verbose_logging:
                    self.logger.info(
                        f"[VERBOSE] Processing WebRTC SDP for media bridge:")
                    self.logger.info(
                        f"  SDP length: {len(session.local_sdp)} bytes")
                ...
                if self.verbose_logging:
                    self.logger.info(
                        f"[VERBOSE] Converted WebRTC SDP to SIP SDP")
                ...
                if audio_info:
                    ...
                    if self.verbose_logging:
                        self.logger.info(
                            f"[VERBOSE] RTP endpoint info extracted:")
                        self.logger.info(
                            f"  Address: {
                    ...
                    if self.verbose_logging:
                        self.logger.warning(
                            f"[VERBOSE] No audio info found in SDP")
            else:
                if self.verbose_logging:
                    self.logger.warning(
                        f"[VERBOSE] No local SDP available in session")
            ...
            if rtp_ports:
                ...
                if self.verbose_logging:
                    self.logger.info(
                        f"[VERBOSE] RTP ports allocated: {rtp_ports}")
            else:
                self.logger.error(
                    f"Failed to allocate RTP ports for WebRTC call {call_id}")
                return None
            ...
            if self.pbx_core.auto_attendant and target_extension == self.pbx_core.auto_attendant.get_extension():
                if self.verbose_logging:
                    self.logger.info(
                        f"[VERBOSE] Target is auto attendant, starting session")

                # Mark as auto attendant call
                call.auto_attendant_active = True

                # Start auto attendant session
                aa_session = self.pbx_core.auto_attendant.start_session(
                    call_id, from_extension)
                call.aa_session = aa_session

                if self.verbose_logging:
                    self.logger.info(
                        f"[VERBOSE] Auto attendant session started")
                    self.logger.info(
                ...
                if aa_session.get(
                        'action') == 'play' and aa_session.get('file'):
            ...
            if re.match(voicemail_pattern, target_extension):
                if self.verbose_logging:
                    self.logger.info(
                        f"[VERBOSE] Target is voicemail access: {target_extension}")
                ...
                if self.pbx_core.voicemail_system:
                    mailbox = self.pbx_core.voicemail_system.get_mailbox(
                        target_mailbox)

                    if self.verbose_logging:
                        self.logger.info(
                            f"[VERBOSE] Setting up voicemail IVR for WebRTC")
                    ...
                    voicemail_ivr = VoicemailIVR(
                        self.pbx_core.voicemail_system, target_mailbox)
                    ...
                    if hasattr(self.pbx_core,
                               'cdr_system') and self.pbx_core.cdr_system:
                    ...
                    if call.caller_rtp and isinstance(call.caller_rtp, dict):
                        ...
                        if caller_address and caller_port and call.rtp_ports:
                            if self.verbose_logging:
                                self.logger.info(
                                    f"[VERBOSE] Starting voicemail IVR session with RTP")
                                self.logger.info(
                                    f"  Caller RTP: {caller_address}:{caller_port}")
                                self.logger.info(
                                    f"  Local RTP port: {
                                        call.rtp_ports[0]}")
                            ...
                            if self.verbose_logging:
                                self.logger.info(
                                    f"[VERBOSE] Voicemail IVR thread started successfully")
                        else:
                            self.logger.warning(
                                f"Incomplete RTP info for WebRTC voicemail call {call_id}")
                            if self.verbose_logging:
                                self.logger.warning(
                                    f"[VERBOSE] Missing RTP data:")
                                self.logger.warning(
                                    f"  caller_address: {caller_address}")
                                self.logger.warning(
                                    f"  caller_port: {caller_port}")
                                self.logger.warning(
                                    f"  rtp_ports: {call.rtp_ports}")
                    else:
                        self.logger.warning(
                    ...
                    self.logger.error(
                        f"Voicemail system not available for WebRTC call {call_id}")
                    if self.verbose_logging:
                        self.logger.error(
                            f"[VERBOSE] pbx_core.voicemail_system is None")
            ...
            if self.verbose_logging:
                self.logger.info(
                    f"[VERBOSE] ===== Call initiation SUCCESSFUL =====")
            ...
            if self.verbose_logging:
                self.logger.error(
                    f"[VERBOSE] ===== Call initiation FAILED =====")
                self.logger.error(
    ...
    def receive_call(
        ...
        self.logger.info(
            f"Routing incoming call {call_id} to WebRTC session {session_id}")
        ...
        try:
            ...
            if caller_sdp:
                ...
                self.logger.debug(
                    f"Converted SIP SDP to WebRTC SDP for session {session_id}")
            ...
            self.logger.info(
                f"Incoming call {call_id} routed to WebRTC session {session_id}")
            self.logger.info(
                f"Client should be notified via signaling channel to accept/reject call")
    ...
    def answer_call(self, session_id: str, webrtc_signaling=None) -> bool:
        ...
        try:
            ...
            if not session or not session.call_id:
                self.logger.error(
                    f"No call associated with session {session_id}")
                return False
