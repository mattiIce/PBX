...
class KarisLawCompliance:
    ...
    def __init__(self, pbx_core, config: dict = None):
        ...
        self.auto_notify = self.config.get('features', {}).get('karis_law', {}).get('auto_notify', True)
        self.require_location = self.config.get('features', {}).get('karis_law', {}).get('require_location', True)
        
        # Emergency trunk configuration
        self.emergency_trunk_id = self.config.get('features', {}).get('karis_law', {}).get('emergency_trunk_id')
        ...
        if self.enabled:
            ...
            self.logger.info(f"Location requirement: {'ENABLED' if self.require_location else 'DISABLED'}")
        else:
    ...
    def handle_emergency_call(self, caller_extension: str, dialed_number: str, 
                             call_id: str, from_addr: tuple) -> Tuple[bool, Dict]:
        """
        ...
        if location_info:
            self.logger.critical(f"Location: {location_info.get('dispatchable_location', 'Unknown')}")
        else:
    ...
    def validate_compliance(self) -> Dict:
        ...
        if hasattr(self.pbx_core, 'trunk_system'):
            ...
            if self.emergency_trunk_id:
                ...
                if not trunk:
                    results['compliant'] = False
                    results['errors'].append(f"Emergency trunk '{self.emergency_trunk_id}' not found")
                elif not trunk.can_make_call():
                    results['warnings'].append(f"Emergency trunk '{self.emergency_trunk_id}' is not available")
