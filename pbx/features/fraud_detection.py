...
class FraudDetectionSystem:
    ...
    def __init__(self, config=None):
        ...
        if self.enabled:
            ...
            self.logger.info(f"  Max international calls/day: {self.max_international_calls_per_day}")
    ...
    def analyze_call(self, call_data: Dict) -> Dict:
        ...
        return {
            'fraud_score': fraud_score,
            'alerts': alerts,
            'recommendation': 'block' if fraud_score > 0.8 else 'monitor' if fraud_score > 0.5 else 'allow'
        }
    ...
    def _check_international_calls(self, extension: str, destination: str) -> tuple:
        ...
        intl_calls = [
            c for c in self.call_history.get(extension, [])
            if c['timestamp'] > cutoff and 
            (c['destination'].startswith('+') or c['destination'].startswith('011'))
        ]
    ...
    def _check_cost_patterns(self, extension: str) -> tuple:
        ...
        cutoff = datetime.now() - timedelta(days=1)
        daily_cost = sum(
            c['cost'] for c in self.call_history[extension]
    ...
    def get_alerts(self, extension: Optional[str] = None, 
    ...
    def cleanup_old_data(self, days: int = 30):
        ...
        for extension in self.call_history:
            self.call_history[extension] = [
                c for c in self.call_history[extension]
                if c['timestamp'] > cutoff
            ]
