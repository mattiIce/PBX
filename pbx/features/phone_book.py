...
class PhoneBook:
    ...
    def _create_table(self):
        ...
        for index_sql in indexes:
            self.database._execute_with_context(index_sql, "phone_book index creation", critical=False)
    ...
    def add_entry(self, extension: str, name: str, department: str = None,
                  email: str = None, mobile: str = None, 
                  office_location: str = None, ad_synced: bool = False) -> bool:
        """
    ...
    def search(self, query: str, max_results: int = 50) -> List[Dict]:
        ...
        for entry in self.entries.values():
            if (query_lower in entry['name'].lower() or
                query_lower in entry['extension'] or
    ...
    def sync_from_ad(self, ad_integration, extension_registry) -> int:
        ...
        if self.database and self.database.enabled and EXTENSION_DB_AVAILABLE:
            try:
                ...
                # extension_registry for ANY database issue (connection, permissions, missing table, etc.)
                self.logger.error(f"Error syncing from extensions database: {type(e).__name__}: {e}")
