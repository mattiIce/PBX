...
class PhoneBook:
    ...
    def __init__(self, config: dict, database=None):
        ...
        self.auto_sync_from_ad = config.get(
        ...
        if self.enabled:
            ...
            if self.auto_sync_from_ad:
                self.logger.info(
                    "Phone book will auto-sync from Active Directory")
    ...
    def _create_table(self):
        ...
        success = self.database._execute_with_context(
            table_sql, "phone_book table creation")
    ...
    def _load_from_database(self):
        ...
        try:
            ...
            self.logger.info(
                f"Loaded {len(self.entries)} phone book entries from database")
    ...
    def add_entry(
        ...
        if self.database and self.database.enabled:
            try:
                ...
                params = (extension, name, department, email, mobile,
                          office_location, ad_synced, datetime.now())
                success = self.database.execute(query, params)

                if success:
                    self.logger.info(
                        f"Added/updated phone book entry: {extension} - {name}")
                return success
            except Exception as e:
    ...
    def remove_entry(self, extension: str) -> bool:
        ...
        if self.database and self.database.enabled:
            query = "DELETE FROM phone_book WHERE extension = %s" if self.database.db_type == 'postgresql' else \
                    "DELETE FROM phone_book WHERE extension = ?"
    ...
    def search(self, query: str, max_results: int = 50) -> List[Dict]:
        ...
        for entry in self.entries.values():
            if (query_lower in entry['name'].lower() or query_lower in entry['extension'] or (
                    entry.get('department') and query_lower in entry['department'].lower())):
    ...
    def sync_from_ad(self, ad_integration, extension_registry) -> int:
        ...
        if not ad_integration or not ad_integration.enabled:
            self.logger.warning(
                "Active Directory integration not available for phone book sync")
            return 0
