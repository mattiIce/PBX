...
class DNDRule:
    ...
    def __init__(self, rule_id: str, extension: str, rule_type: str, config: dict):
        ...
        self.priority = config.get('priority', 0)  # Higher priority rules override lower
...
class CalendarMonitor:
    ...
    def start(self):
        ...
        if not self.outlook or not self.outlook.enabled:
            self.logger.warning("Calendar monitoring cannot start: Outlook integration not available")
            return
            ...
            try:
                ...
                for event in events:
                    ...
                    if event_start <= now <= event_end:
                        ...
                        if show_as in ['busy', 'outofoffice', 'tentative'] and response_status in ['accepted', 'organizer', 'tentativelyaccepted']:
                            in_meeting = True
                            current_meeting = {
                ...
                if in_meeting and not previous_state:
                    ...
                    self.logger.info(f"Extension {extension} entered meeting: {current_meeting['subject']}")
...
class DNDScheduler:
    ...
    def __init__(self, presence_system=None, outlook_integration=None, config: dict = None):
        ...
        self.previous_statuses = {}  # extension -> PresenceStatus (for restoration)
    ...
    def set_manual_override(self, extension: str, status: PresenceStatus, duration_minutes: int = None):
        """
    ...
    def _check_all_rules(self):
        ...
        for extension in list(set(list(self.rules.keys()) + list(self.calendar_monitor.extension_email_map.keys()))):
            try:
                ...
                # Apply or remove DND
                if should_dnd and current_status not in [PresenceStatus.DO_NOT_DISTURB, PresenceStatus.IN_MEETING]:
                    ...
                    if self.calendar_dnd_enabled and self.calendar_monitor.is_in_meeting(extension)[0]:
                        self.presence_system.set_status(extension, PresenceStatus.IN_MEETING, "In calendar meeting")
                    else:
                        self.presence_system.set_status(extension, PresenceStatus.DO_NOT_DISTURB, "Auto-DND (scheduled)")
                    
                elif not should_dnd and current_status in [PresenceStatus.DO_NOT_DISTURB, PresenceStatus.IN_MEETING]:
    ...
    def get_status(self, extension: str) -> dict:
        ...
        if self.presence_system:
            user = self.presence_system.users.get(extension)
            if user:
                status['dnd_active'] = user.status in [PresenceStatus.DO_NOT_DISTURB, PresenceStatus.IN_MEETING]
...
def get_dnd_scheduler(presence_system=None, outlook_integration=None, config: dict = None) -> DNDScheduler:
