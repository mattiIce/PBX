...
class RecordingRetentionManager:
    ...
    def __init__(self, config=None):
        ...
        self.compliance_retention = retention_config.get('compliance_retention_days', 2555)  # 7 years
    ...
    def _load_policies(self):
        ...
        policies = self.config.get('features', {}).get('recording_retention', {}).get('policies', [])
    ...
    def get_retention_period(self, recording_id: str, 
    ...
    def cleanup_old_recordings(self, dry_run: bool = True) -> Dict:
        ...
        if not dry_run:
            ...
            self.logger.info(f"Deleted {len(deleted_files)} recordings ({deleted_size / 1024 / 1024:.2f} MB)")
        else:
            self.logger.info(f"Would delete {len(deleted_files)} recordings ({deleted_size / 1024 / 1024:.2f} MB)")
