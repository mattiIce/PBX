...
class SessionBorderController:
    ...
    def _normalize_sip_message(self, message: Dict) -> Dict:
        ...
        if 'method' in normalized:
            valid_methods = ['INVITE', 'ACK', 'BYE', 'CANCEL', 'REGISTER', 
    ...
    def allocate_relay(self, call_id: str, codec: str) -> Dict:
        ...
        if len(self.relay_port_pool) < 2:
            return {
                'success': False,
                'reason': 'No relay ports available'
            }
    ...
    def _check_rate_limit(self, ip: str) -> bool:
        ...
        if len(self.request_counts[ip]) >= self.rate_limit:
            self.logger.warning(f"Rate limit exceeded for {ip}: {len(self.request_counts[ip])} requests in {self.rate_limit_window}s")
            return False
