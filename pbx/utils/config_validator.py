...
class ConfigValidator:
    ...
    def _validate_server_config(self):
        ...
        if rtp_start >= rtp_end:
            self.errors.append(
                f"RTP port range invalid: start ({rtp_start}) >= end ({rtp_end})"
            )
        ...
        if not external_ip:
            self.warnings.append(
                "external_ip not set. This may cause issues with SIP signaling."
            )
        elif external_ip == "0.0.0.0":
            self.warnings.append(
        ...
        if db_type == "postgresql":
            ...
            for field in required:
                ...
                if isinstance(value, str) and value.startswith("${") and value.endswith("}"):
                    ...
                    self.errors.append(
                        f"PostgreSQL config missing required field: {field}"
                    )
            ...
            self.warnings.append(
                "Using SQLite database. PostgreSQL is recommended for production."
            )
    ...
    def _validate_api_config(self):
        ...
        if ssl_enabled:
            ...
            if not cert_file or not key_file:
                self.errors.append(
                    "SSL enabled but cert_file or key_file not specified"
                )
    ...
    def _validate_security_config(self):
        ...
        if not fips_mode:
            self.warnings.append(
                "FIPS mode is disabled. Enable for government-grade encryption."
            )
    ...
    def _validate_extensions_config(self):
        ...
        for ext in extensions:
            ...
            if len(password) < 8:
                self.warnings.append(
                    f"Extension {number}: password is too short (< 8 characters)"
                )
            
            for pattern in weak_patterns:
                if re.match(pattern, password.lower()):
                    self.warnings.append(
                        f"Extension {number}: weak password detected"
                    )
                    break
    ...
    def _validate_codecs_config(self):
        ...
        enabled_codecs = [
            name for name, cfg in codecs_config.items()
            if isinstance(cfg, dict) and cfg.get("enabled", False)
        ]
        
        if not enabled_codecs:
            self.errors.append(
                "No audio codecs enabled. At least one codec must be enabled."
            )
        
        # Check for G.711 (PCMU/PCMA) which should always be available
        if not any(name.startswith("g711") or name in ["pcmu", "pcma"] 
                   for name in enabled_codecs):
            self.warnings.append(
    ...
    def _validate_production_readiness(self):
        ...
        if "example" in server_name.lower():
            self.warnings.append(
                "Server name contains 'example'. Update for production."
            )
