...
class GracefulShutdownHandler:
    ...
    def _wait_for_calls_to_complete(self, timeout: int = 20):
        ...
        while time.time() - start_time < timeout:
            ...
            logger.info(f"Waiting for {call_count} active call(s) to complete... ({remaining:.1f}s remaining)")
...
class ConnectionRetry:
    ...
    def __next__(self):
        ...
        if self.attempt > 0:
            ...
            logger.info(f"Retry attempt {self.attempt}/{self.max_retries} after {delay:.1f}s delay...")
            time.sleep(delay)
