...
def run_all_tests(tests_dir="tests", log_failures=True):
    ...
    if not os.path.isabs(tests_dir):
        tests_dir = os.path.join(
            os.path.dirname(__file__), '..', '..', tests_dir)
        ...
        for test_file in test_files:
            ...
            try:
                ...
                if spec is None or spec.loader is None:
                    ...
                    total_failed += 1
                    test_results.append(
                        (test_file, False, "Failed to load module"))
                    print()
                    continue
                ...
                if hasattr(module, 'run_all_tests'):
                    ...
                    # If test returned None/False, check output for success
                    # indicators
                    if not test_passed and test_stdout:
                        ...
                        if ('0 failed' in output_lower or
                            ...
                            # Also check that there's no explicit failure
                            # indicator
                    ...
                    if test_passed:
                        ...
                        if log_file:
                            log_file.write(
                                f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] FAILED: {test_file}\n")
                            log_file.write(
                                f"  Return value: {success} (expected True)\n")
                            if test_stdout:
                                log_file.write(f"\n  Test Output:\n")
                    ...
                    print(
                        f"  ⚠ {test_file} has no run_all_tests() function (skipped)")
                    total_skipped += 1
                    test_results.append(
                        (test_file, None, "No run_all_tests function"))
                ...
                if log_file:
                    log_file.write(
                        f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] ERROR: {test_file}\n")
            ...
            try:
                ...
                log_file.write(
                    f"Test Run Completed: {
                        datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
                log_file.write(
                    f"Summary: {total_passed} passed, {total_failed} failed, {total_skipped} skipped\n")
                log_file.write("=" * 80 + "\n")
