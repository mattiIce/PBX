...
export async function loadCodecStatus() {
    try {
        const API_BASE = getApiBaseUrl();
        ...
        const container = document.getElementById('codec-status');
...
export async function loadDTMFConfig() {
    try {
        const API_BASE = getApiBaseUrl();
        ...
        const modeSelect = document.getElementById('dtmf-mode');
        if (modeSelect) modeSelect.value = data.mode || 'rfc2833';

        const threshold = document.getElementById('dtmf-threshold');
        if (threshold) threshold.value = data.threshold || -30;
    } catch (error) {
        console.error('Error loading DTMF config:', error);
    }
}

export async function saveDTMFConfig() {
    try {
        const API_BASE = getApiBaseUrl();
        const config = {
            mode: document.getElementById('dtmf-mode')?.value || 'rfc2833',
            threshold: parseInt(document.getElementById('dtmf-threshold')?.value || '-30')
        };
