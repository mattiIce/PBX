...
class SecurityComplianceChecker:
    ...
    def check_soc2_compliance(self) -> Dict:
        ...
        try:
            ...
            # Print summary
            self.print_status(
                "SOC 2 Controls Total",
                total > 0,
                f"Total: {total} controls"
            )
            ...
            if implemented < total:
                soc2_results['compliant'] = False
                soc2_results['issues'].append(
                    f"{total - implemented} controls not yet implemented"
                )
    ...
    def check_security_configuration(self) -> Dict:
        ...
        try:
            ...
            self.print_status(
                "Failed login limit configured",
                failed_login_ok,
                f"Max attempts: {max_failed}"
            )
            ...
            if not enable_tls:
                security_results['issues'].append(
                    "TLS not enabled - recommended for production"
                )
            ...
            if not enable_srtp:
                security_results['issues'].append(
                    "SRTP not enabled - recommended for production"
                )
            ...
            if not api_auth:
                security_results['issues'].append(
                    "API authentication not required - security risk"
                )
    ...
    def print_summary(self):
        ...
        fips_status = "✓ COMPLIANT" if self.results['overall']['fips_compliant'] else "✗ NON-COMPLIANT"
        soc2_status = "✓ COMPLIANT" if self.results['overall']['soc2_compliant'] else "✗ NON-COMPLIANT"
