...
def main():
    ...
    parser.add_argument(
        '--all',
        action='store_true',
        help='Generate all MOH tracks (default)'
    )
    parser.add_argument(
        '--melody',
        action='store_true',
        help='Generate only simple melody'
    )
    parser.add_argument(
        '--ambient',
        action='store_true',
        help='Generate only ambient tones'
    )
    parser.add_argument(
        '--arpeggio',
        action='store_true',
        help='Generate only arpeggio'
    )
    parser.add_argument(
        '--pad',
        action='store_true',
        help='Generate only soft pad'
    )
    parser.add_argument(
        '--chimes',
        action='store_true',
        help='Generate only gentle chimes'
    )
    ...
    if generate_all or args.melody:
        generate_simple_melody(
            os.path.join(args.output_dir, 'melody.wav'),
            args.duration
        )
    
    if generate_all or args.ambient:
        generate_ambient_tones(
            os.path.join(args.output_dir, 'ambient.wav'),
            args.duration
        )
    
    if generate_all or args.arpeggio:
        generate_arpeggio(
            os.path.join(args.output_dir, 'arpeggio.wav'),
            args.duration
        )
    
    if generate_all or args.pad:
        generate_soft_pad(
            os.path.join(args.output_dir, 'pad.wav'),
            args.duration
        )
    
    if generate_all or args.chimes:
        generate_gentle_chimes(
            os.path.join(args.output_dir, 'chimes.wav'),
            args.duration
        )
