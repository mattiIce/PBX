...
class ADIntegrationTester:
    ...
    def test_configuration(self) -> bool:
        ...
        if not enabled:
            self.print_test("AD integration enabled", "FAIL", 
        ...
        for field_name, config_key in required_fields.items():
            ...
            if not value:
                self.print_test(f"Required field: {field_name}", "FAIL", 
                              f"Missing or empty: {config_key}")
                all_fields_present = False
            else:
                # Don't show password in output
                display_value = "***" if 'password' in field_name.lower() else value
                self.print_test(f"Required field: {field_name}", "PASS", 
                              f"{display_value}")
        ...
        if user_search_base:
            ...
            self.print_test("User search base configured", "WARN", 
                          "Not set, will use base_dn (may be slower)")
        ...
        if auto_provision:
            ...
            self.print_test("Auto-provision enabled", "WARN", 
                          "Extensions won't be created automatically")
        ...
        if use_ssl:
            ...
            self.print_test("SSL/TLS enabled", "WARN", 
    ...
    def test_ldap3_available(self) -> bool:
        ...
        try:
            ...
            self.print_test("ldap3 library installed", "FAIL", 
    ...
    def test_connection(self) -> bool:
        ...
        ad_config = {
            ...
            'integrations.active_directory.auto_provision': self.config.get('integrations.active_directory.auto_provision'),
        ...
        if self.ad.connect():
            self.print_test("Connect to AD server", "PASS", "Successfully connected and authenticated")
            return True
        else:
            self.print_test("Connect to AD server", "FAIL", 
    ...
    def test_user_search(self) -> Tuple[bool, List[Dict]]:
        ...
        try:
            ...
            if len(users) == 0:
                self.print_test("Search for users", "WARN", 
                              "No users found with telephoneNumber attribute")
                return True, []
            else:
                self.print_test("Search for users", "PASS", 
                              f"Found {len(users)} users with phone numbers")
                return True, users
    ...
    def test_user_attributes(self, users: List[Dict]) -> bool:
        ...
        for i, user in enumerate(users[:check_count], 1):
            ...
            if user['email']:
                ...
                print(f"  {Colors.YELLOW}⚠{Colors.RESET} Missing email (voicemail notifications disabled)")
        ...
        if all_valid:
            self.print_test("User attributes", "PASS", 
                          f"All checked users have required attributes")
        else:
            self.print_test("User attributes", "WARN", 
    ...
    def test_extension_sync(self, users: List[Dict]) -> bool:
        ...
        if not users:
            self.print_test("Extension sync readiness", "WARN", 
        ...
        self.print_test("Syncable users", "INFO", 
                      f"{len(syncable_users)} users can be synced")
        
        if skipped_users:
            self.print_test("Skipped users", "INFO", 
                          f"{len(skipped_users)} users will be skipped")
        ...
        if conflicts:
            print()
            self.print_test("Extension conflicts", "INFO", 
        ...
        self.print_test("Extension sync check", "PASS", 
                      "Ready to sync users")
    ...
    def test_authentication(self, users: List[Dict]) -> bool:
        ...
        self.print_test("Authentication capability", "INFO", 
