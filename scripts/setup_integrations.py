...
class IntegrationSetup:
    ...
    INTEGRATIONS: ClassVar[dict[str, dict]] = {
        ...
        "matrix": {
            ...
            "env_vars": {"MATRIX_BOT_PASSWORD": "your-bot-password"},
        },
        "espocrm": {
            "name": "EspoCRM",
            ...
            "env_vars": {"ESPOCRM_API_KEY": "your-api-key"},
        },
    }
    ...
    def ensure_env_file(self) -> None:
        ...
        if not self.env_path.exists():
            if self.env_example_path.exists():
                # Copy from example
                with (
                    self.env_example_path.open() as src,
                    self.env_path.open("w") as dst,
                ):
                    dst.write(src.read())
    ...
    def enable_integration(self, integration_name: str, config_updates: dict | None = None) -> bool:
        """Enable an integration with optional custom configuration"""
        if integration_name not in self.INTEGRATIONS:
            print(f"❌ Error: Unknown integration '{integration_name}'")
    ...
    def disable_integration(self, integration_name: str) -> bool:
        ...
        if "integrations" in config and integration_name in config["integrations"]:
            ...
            self.save_config(config)
            print(f"✅ {integration['name']} integration disabled!")
        else:
            print(f"ℹ️  {integration['name']} was not configured")
    ...
    def interactive_setup(self) -> None:
        ...
        print("\n" + "=" * 60)
        print("🎯 PBX Integration Setup Wizard")
        print("=" * 60)
    ...
    def configure_integration_interactive(self, integration_name: str) -> None:
        ...
        if integration_name == "jitsi":
            print("\nJitsi will use your local installation by default (https://localhost:8443).")
            ...
            if use_local == "n":
                ...
                config_updates["server_url"] = "https://localhost:8443"
            ...
            print("\nℹ️  Bot password will be set in .env file (MATRIX_BOT_PASSWORD)")
            ...
            print(
                "\nEspoCRM will use your local installation by default (https://localhost:8001/api/v1)."
            )
            use_local = input("Use local EspoCRM? [Y/n]: ").strip().lower()
            if use_local == "n":
                api_url = input(
                    "Enter EspoCRM API URL (e.g., https://crm.yourcompany.com/api/v1): "
                ).strip()
                if api_url:
                    config_updates["api_url"] = api_url
            else:
                config_updates["api_url"] = "https://localhost:8001/api/v1"
...
Examples:
  ...
  %(prog)s --status
    ...
    parser.add_argument(
        "--status", "-s", action="store_true", help="Show status of all integrations"
    )

    parser.add_argument(
        "--base-path", metavar="PATH", help="Base path to PBX installation (default: auto-detect)"
    )
