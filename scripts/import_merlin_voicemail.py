...
def main():
    ...
    parser.add_argument('--parse-filenames', action='store_true',
    ...
    if pins:
        ...
        imported, skipped, errors = import_voicemail_pins(
            pins, config, database, args.dry_run
        )
        total_imported += imported
        total_skipped += skipped
        total_errors += errors
    ...
    if args.greetings_dir:
        ...
        imported, errors = import_greetings(
            args.greetings_dir, config, database, args.dry_run
        )
        total_imported += imported
        total_errors += errors
