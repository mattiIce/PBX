...
class PhoneProvisioningSetup:
    ...
    def add_device_to_config(self, mac, extension, vendor, model):
        ...
        for device in devices:
            if device.get('mac', '').replace(':', '').replace('-', '').replace('.', '').lower() == mac_normalized:
    ...
    def setup_provisioning_settings(self):
        ...
        current_port = api_section.get('port', 8080)
        ...
        custom_dir = ''
        if has_custom:
            custom_dir = self.get_input("Enter path to custom templates directory", "custom_templates")
    ...
    def add_phone_device(self):
        ...
        if not vendors:
            print("❌ No phone vendors available. Make sure PBX is running or templates are configured.")
            return None
    ...
    def register_devices(self):
        ...
        for i, device in enumerate(self.devices_to_register, 1):
            ...
            if method == 'api':
                if self.register_device_api(device['mac'], device['extension'], 
                    ...
                    fail_count += 1
                    # Fallback to config
                    if self.add_device_to_config(device['mac'], device['extension'],
                                                device['vendor'], device['model']):
