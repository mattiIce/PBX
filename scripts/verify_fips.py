...
from pbx.utils.encryption import get_encryption, CRYPTO_AVAILABLE, FIPSEncryption
...
def check_system_fips():
    ...
    try:
        with open('/proc/sys/crypto/fips_enabled', 'r') as f:
            ...
            print_status("Kernel FIPS mode", fips_enabled, 
                        f"Value: {fips_enabled}")
            if not fips_enabled:
                all_passed = False
    except FileNotFoundError:
        print_status("Kernel FIPS mode", False, 
                    "File /proc/sys/crypto/fips_enabled not found")
        all_passed = False
    ...
    try:
        ...
        if fips_mode_func:
            ...
            print_status("Python hashlib FIPS mode", fips_mode == 1,
                        f"Value: {fips_mode}")
            if fips_mode != 1:
                all_passed = False
        else:
            print_status("Python hashlib FIPS mode", False,
                        "get_fips_mode() not available")
            all_passed = False
    except Exception as e:
...
def check_cryptography_library():
    ...
    print_status("Cryptography library installed", CRYPTO_AVAILABLE,
    ...
    try:
        ...
        if 'FIPS: True' in backend_name:
            ...
            print_status("Backend FIPS mode", False,
                        "Backend is not using FIPS-validated module")
            all_passed = False
        else:
            print_status("Backend FIPS mode", False,
                        "Cannot determine FIPS status from backend")
    except Exception as e:
...
def check_pbx_configuration():
    ...
    try:
        ...
        print_status("FIPS mode enabled in config", fips_mode,
                    f"security.fips_mode = {fips_mode}")
        if not fips_mode:
            all_passed = False
        
        # Check enforce FIPS
        enforce_fips = config.get('security.enforce_fips', False)
        print_status("FIPS enforcement enabled", enforce_fips,
                    f"security.enforce_fips = {enforce_fips}")
        
        # Check password policy
        min_length = config.get('security.password.min_length', 0)
        print_status("Password minimum length >= 12", min_length >= 12,
        ...
        print_status("TLS enabled (recommended)", enable_tls,
                    "For SIPS (SIP over TLS)")
        
        # Check SRTP configuration
        enable_srtp = config.get('security.enable_srtp', False)
        print_status("SRTP enabled (recommended)", enable_srtp,
                    "For encrypted media streams")
...
def test_encryption_operations():
    ...
    try:
        ...
        try:
            ...
            print_status("Password hashing (PBKDF2-HMAC-SHA256)", True,
                        "600,000 iterations")
        except Exception as e:
        ...
        try:
            ...
            print_status("SHA-256 hashing", len(hash_result) == 64,
        ...
        try:
            ...
            print_status("Secure token generation", len(token) > 0,
...
def check_dependencies():
    ...
    for package, min_version in optional_packages.items():
        try:
            ...
            print_status(f"{package} installed (optional)", False,
