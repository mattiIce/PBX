...
def test_compliance_checker_exists():
    ...
    script_path = os.path.join(os.path.dirname(__file__), "..", "scripts", "security_compliance_check.py")
...
def test_compliance_checker_help():
    ...
    assert "FIPS" in result.stdout or "compliance" in result.stdout.lower(), "Help should mention FIPS or compliance"
...
def test_compliance_checker_json_output():
    ...
    assert data["overall"]["status"] in ["COMPLIANT", "COMPLIANT_WITH_WARNINGS", "NON_COMPLIANT"], \
...
def test_wrapper_script_exists():
    ...
    script_path = os.path.join(os.path.dirname(__file__), "..", "scripts", "run_compliance_check.sh")
