...
def test_dtmf_detection():
    ...
    test_sequence = '12345'
    sequence_samples = generator.generate_sequence(
        test_sequence, tone_ms=100, gap_ms=50)
...
def test_ad_group_permissions_mapping():
    ...
    assert permissions.get(
        'admin') is True, "Admin permission should be granted"
    assert permissions.get(
        'manage_extensions') is True, "Manage extensions permission should be granted"
    assert permissions.get(
        'view_cdr') is True, "View CDR permission should be granted"
    ...
    user_groups = [
        ...
        'CN=Support,OU=Groups,DC=test,DC=local']
    permissions = ad._map_groups_to_permissions(user_groups)
    assert permissions.get(
        'external_calling') is True, "External calling should be granted"
    assert permissions.get(
        'international_calling') is True, "International calling should be granted"
    assert permissions.get(
        'call_recording') is True, "Call recording should be granted"
    assert permissions.get(
        'call_queues') is True, "Call queues should be granted"
    assert 'admin' not in permissions, "Admin permission should not be granted"

    # Test case 3: User with no matching groups
    user_groups = [
        'CN=HR,OU=Groups,DC=test,DC=local',
        'CN=Finance,OU=Groups,DC=test,DC=local']
    permissions = ad._map_groups_to_permissions(user_groups)
    assert len(
        permissions) == 0, "No permissions should be granted for non-configured groups"

    # Test case 4: User with CN-only group names (short format)
    user_groups = ['Sales', 'Executives']  # Short format without full DN
    permissions = ad._map_groups_to_permissions(user_groups)
    assert permissions.get(
        'external_calling') is True, "Should match by CN short name"
    assert permissions.get(
        'vip_status') is True, "Should match by CN short name"
    ...
    assert len(
        permissions) == 0, "No permissions when group_permissions not configured"
