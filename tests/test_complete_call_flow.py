...
"""
    ...
    def test_call_flow_sequence(self):
        ...
        pbx_ip = '192.168.1.1'
        pbx_rtp_port = 10000
        sdp_to_callee = SDPBuilder.build_audio_sdp(
            pbx_ip, pbx_rtp_port, '12345')
        self.assertIn(f'm=audio {pbx_rtp_port}', sdp_to_callee)
        self.assertIn(f'c=IN IP4 {pbx_ip}', sdp_to_callee)
        print(
            f"   ✓ SDP to callee contains PBX endpoint: {pbx_ip}:{pbx_rtp_port}")

        # PBX generates SDP for caller with its own RTP endpoint
        sdp_to_caller = SDPBuilder.build_audio_sdp(
            pbx_ip, pbx_rtp_port, '12345')
        self.assertIn(f'm=audio {pbx_rtp_port}', sdp_to_caller)
        print(
            f"   ✓ SDP to caller contains PBX endpoint: {pbx_ip}:{pbx_rtp_port}")
