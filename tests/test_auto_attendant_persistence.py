...
class TestAutoAttendantPersistence(unittest.TestCase):
    ...
    def test_initial_config_saved_to_db(self):
        ...
        cursor = conn.cursor()
        cursor.execute('SELECT enabled, extension, timeout, max_retries FROM auto_attendant_config WHERE id = 1')
        row = cursor.fetchone()
        conn.close()
    ...
    def test_initial_menu_options_saved_to_db(self):
        ...
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        cursor.execute('SELECT digit, destination, description FROM auto_attendant_menu_options ORDER BY digit')
    ...
    def test_database_tables_created(self):
        ...
        # Check config table
        cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='auto_attendant_config'")
        self.assertIsNotNone(cursor.fetchone())
        
        # Check menu options table
        cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='auto_attendant_menu_options'")
        self.assertIsNotNone(cursor.fetchone())
