...
def test_phone_book_auto_sync_after_ad_sync():
    ...
    assert phone_book.enabled == True, "Phone book should be enabled"
    assert phone_book.auto_sync_from_ad == True, "Auto sync from AD should be enabled"
    ...
    assert len(phone_book.entries) == 2, f"Should have 2 entries in phone book, got {len(phone_book.entries)}"
