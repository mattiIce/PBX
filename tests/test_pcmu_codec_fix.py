...
def test_pcm_to_pcmu_conversion():
    ...
    try:
        ...
        if len(ulaw_data) == 100:  # Should be half the size (16-bit PCM -> 8-bit μ-law)
            print(f"   ✓ Manual conversion successful: {len(test_pcm_data)} bytes PCM -> {len(ulaw_data)} bytes PCMU")
        else:
            print(f"   ✗ Manual conversion failed: expected 100 bytes, got {len(ulaw_data)} bytes")
            return False
