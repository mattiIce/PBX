...
from pbx.features.crm_integration import (
    CallerInfo, CRMLookupProvider, PhoneBookLookupProvider,
    ActiveDirectoryLookupProvider, ExternalCRMLookupProvider,
    CRMIntegration
)
...
def test_crm_integration_initialization():
    ...
    assert crm.enabled == True, "Should be enabled"
    assert crm.cache_enabled == True, "Cache should be enabled"
...
def test_phone_number_normalization():
    ...
    assert crm._normalize_phone_number('(555) 123-4567') == '5551234567', "Should remove parens and spaces"
    assert crm._normalize_phone_number('+1-555-123-4567') == '15551234567', "Should remove plus and dashes"
...
def test_provider_status():
    ...
    assert status[0]['enabled'] == True, "First provider should be enabled"
...
def run_all_tests():
    ...
    print("\n" + "=" * 70)
    if all(results):
        print(f"✅ All CRM integration tests passed! ({len(results)}/{len(results)})")
        return True
    else:
