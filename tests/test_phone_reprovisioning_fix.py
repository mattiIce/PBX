...
def test_phone_reprovisioning_removes_old_mapping():
    ...
    phones_ext_1001 = phones_db.get_by_extension("1001")
    assert len(phones_ext_1001) == 0, f"Extension 1001 should have 0 phones, but has {len(phones_ext_1001)}"
    
    # Step 4: Verify new mapping exists
    phones_ext_1002 = phones_db.get_by_extension("1002")
    assert len(phones_ext_1002) == 1, f"Extension 1002 should have 1 phone, but has {len(phones_ext_1002)}"
...
def test_phone_reprovisioning_with_mac_then_ip():
    ...
    # Reprovision to extension 1006, but this time without MAC (phone didn't send it)
    success, _ = phones_db.register_phone(
