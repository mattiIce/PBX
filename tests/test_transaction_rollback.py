...
def test_transaction_rollback_on_error():
    ...
    try:
        ...
        # If rollback didn't work, this would fail with "transaction is aborted" error
        result = db.execute("INSERT INTO vip_callers (caller_id, priority_level) VALUES (?, ?)", 
                          ('1234567890', 1))
...
def test_schema_migration_rollback():
    ...
    try:
        ...
        # After schema migration (even with potential errors), we should be able to insert
        result = db.execute("INSERT INTO vip_callers (caller_id, priority_level) VALUES (?, ?)", 
                          ('1112223333', 1))
        assert result is True
