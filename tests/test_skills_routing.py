...
from pbx.features.skills_routing import SkillsBasedRouter, Skill, AgentSkill, SkillRequirement
...
def test_find_best_agents():
    ...
    requirements = [
        {'skill_id': 'python', 'min_proficiency': 4, 'required': True}
    ]
    ...
    print(f"    Top agent: {best_agents[0]['extension']} (score: {best_agents[0]['score']:.2f})")
...
def test_scoring_algorithm():
    ...
    for i, agent in enumerate(best_agents, 1):
        print(f"    {i}. Agent {agent['extension']}: score {agent['score']:.3f}, {len(agent['matching_skills'])} skills")
