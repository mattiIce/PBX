...
def test_hot_desk_session_creation():
    ...
    assert session.auto_logout_enabled == True, "Auto-logout should be enabled by default"
...
def test_hot_desking_initialization():
    ...
    assert hot_desk.enabled == True, "Should be enabled"
    assert hot_desk.auto_logout_timeout == 28800, "Timeout should be 28800"
    assert hot_desk.require_pin == True, "Should require PIN"
...
def test_hot_desk_login_logout():
    ...
    assert hot_desk.is_logged_in('1001') == True, "Extension should be logged in"
    
    # Test logout
    success = hot_desk.logout('device-abc')
    assert success == True, "Logout should succeed"
...
def test_hot_desk_profile_retrieval():
    ...
    assert profile['allow_external'] == True, "allow_external should match"
