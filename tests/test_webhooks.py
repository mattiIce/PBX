...
from pbx.features.webhooks import WebhookSystem, WebhookEvent, WebhookSubscription
...
def test_webhook_subscription():
    ...
    assert subscription.enabled == True, "Should be enabled by default"
    
    # Test event matching
    assert subscription.matches_event('call.started') == True, "Should match call.started"
    assert subscription.matches_event('call.ended') == True, "Should match call.ended"
    ...
    assert wildcard_sub.matches_event('call.started') == True, "Wildcard should match call.started"
    assert wildcard_sub.matches_event('voicemail.new') == True, "Wildcard should match voicemail.new"
    
    # Test disabled subscription
    subscription.enabled = False
    assert subscription.matches_event('call.started') == False, "Disabled subscription should not match"
...
def test_webhook_system_initialization():
    ...
    assert webhook_system.enabled == True, "Should be enabled"
...
def test_webhook_subscription_management():
    ...
    assert subs[0]['enabled'] == True, "Should be enabled"
    
    # Disable subscription
    success = webhook_system.disable_subscription('http://localhost:9999/test')
    assert success == True, "Should disable successfully"
    ...
    assert success == True, "Should enable successfully"
    assert webhook_system.subscriptions[0].enabled == True, "Should be enabled"
